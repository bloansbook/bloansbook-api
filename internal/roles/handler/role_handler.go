...
func (r *RolesHandler) GetAllRoles(c fiber.Ctx) error {
	...
	rolesList, err := r.usecase.GetAllRoles(c.Context(), limit, offset)
	if err != nil {
		return response.Error(c, err.Error(), fiber.StatusInternalServerError)
	}

	return response.Success(c, sysmsg.RoleListFetched, rolesList, fiber.StatusOK)
}
