...
func (a *AuthRepository) GetMe(ctx context.Context, userID uuid.UUID) (*auth.ProfileDTO, error) {
	stmt := `
		...
		flat_perms AS (
			-- All distinct permission codes for this staff member across all roles,
			-- returned as a JSON array of plain strings for the frontend.
			SELECT
				COALESCE(
					json_agg(DISTINCT p.code ORDER BY p.code),
					'[]'::json
				) AS codes
			FROM staff_roles sr
		...
		SELECT
			...
			(SELECT codes FROM flat_perms) AS permissions
		FROM staff s
